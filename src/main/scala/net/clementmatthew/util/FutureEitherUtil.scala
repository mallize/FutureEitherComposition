...
object FutureEitherUtil {
   def applyEither[L, R1, R2](f: (R1) => Future[Either[L, R2]], e: Either[L, R1])(implicit ec: ExecutionContext): Future[Either[L, R2]] = {
      e match {
   ...
   def applyEither1[L, R1, R2, A](f: (R1, A) => Future[Either[L, R2]], e: Either[L, R1], s: A)(implicit ec: ExecutionContext): Future[Either[L, R2]] = {
      e match {
         case Left(l) => Future.successful(Left(l))
         case Right(r) => f(r, s)
      }
